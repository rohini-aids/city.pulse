...
import { Map, List, Plus, Sparkles, Navigation, Search, Loader2, CheckCircle, LogOut, X } from 'lucide-react';
...
const App: React.FC = () => {
    ...
    if ('geolocation' in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (isAuthenticated) {
            // Only center if authenticated to avoid jarring jumps before login
            setViewState(prev => ({
              ...prev,
              center: coords
            }));
  ...
  }, [isAuthenticated]); // Re-check or apply location when auth changes
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setSearchQuery('');
        setAiSummary(null); // Clear old summary when moving
      } else {
  ...
  const handleLogout = () => {
    ...
    setViewState(prev => ({...prev, mode: 'MAP'})); // Reset view
  };
    ...
    <div className="h-screen w-full flex flex-col bg-slate-100 overflow-hidden relative">
      ...
      {notification && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[1000] animate-in slide-in-from-top-4 fade-in duration-300">
          <div className={`px-6 py-3 rounded-full shadow-xl text-white font-medium flex items-center space-x-2 ${
